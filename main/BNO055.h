...
#define ENABLE_FLOAT
static const char *TAG = "BNO055_TESTER";
...
// Estructura global para el BNO055
struct bno055_t bno055_device;

// =================================================================
// FUNCIONES DE DRIVER INTEGRADO (OPTIMIZADO: Timeout de 100 ms)
// =================================================================

BNO055_RETURN_FUNCTION_TYPE bno055_bus_read(unsigned char dev_addr, unsigned char reg_addr, unsigned char *reg_data, unsigned char r_len) {
    // Timeout reducido a 100 ms
    esp_err_t ret = i2c_master_write_read_device(I2C_MASTER_NUM, dev_addr, &reg_addr, 1, reg_data, r_len, pdMS_TO_TICKS(100));
    if (ret != ESP_OK) {
...
BNO055_RETURN_FUNCTION_TYPE bno055_bus_write(unsigned char dev_addr, unsigned char reg_addr, unsigned char *reg_data, unsigned char wr_len) {
    uint8_t write_buffer[wr_len + 1];
    write_buffer[0] = reg_addr;
    memcpy(&write_buffer[1], reg_data, wr_len);
    // Timeout reducido a 100 ms
    esp_err_t ret = i2c_master_write_to_device(I2C_MASTER_NUM, dev_addr, write_buffer, wr_len + 1, pdMS_TO_TICKS(100));
    if (ret != ESP_OK) {
...
void bno055_delay_msec(BNO055_MDELAY_DATA_TYPE delay_in_msec) {
    vTaskDelay(pdMS_TO_TICKS(delay_in_msec));
}


BNO055_RETURN_FUNCTION_TYPE bno055_read_euler_hrp(struct bno055_euler *euler_data) {
    unsigned char data[6];
    if (bno055_bus_read(bno055_device.dev_addr, BNO055_EUL_HEADING_LSB_ADDR, data, 6) != SUCCESS) return ERROR1;
    euler_data->h = (BNO055_S16)((data[1] << 8) | data[0]);
    euler_data->r = (BNO055_S16)((data[3] << 8) | data[2]);
    euler_data->p = (BNO055_S16)((data[5] << 8) | data[4]);
    return SUCCESS;
}



// Diagnóstico y escáner (se mantienen igual para referencia)
void i2c_scanner() {
    ...
    for (uint8_t addr = 1; addr < 127; addr++) {
        ...
        i2c_master_stop(cmd);
        // Timeout de 200ms para el escaneo
...
void test_chip_id(uint8_t addr) {
    uint8_t id = 0;
    esp_err_t ret = i2c_master_write_read_device(I2C_MASTER_NUM, addr, (uint8_t[]){BNO055_CHIP_ID_ADDR}, 1, &id, 1, pdMS_TO_TICKS(100));
    if (ret == ESP_OK) {
        ESP_LOGI(TAG, "Dirección 0x%02X respondió con Chip ID: 0x%02X", addr, id);
        if (id == 0xA0) {
            ESP_LOGI(TAG, "BNO055 detectado correctamente.");
        } else {
            ESP_LOGW(TAG, "ID inesperado. El sensor respondió pero no está listo.");
        }
    } else {
        ESP_LOGE(TAG, " Fallo al leer ID en 0x%02X: %s", addr, esp_err_to_name(ret));
    }
}
